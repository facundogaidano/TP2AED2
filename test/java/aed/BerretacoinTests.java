...
public class BerretacoinTests {
    ...
    @Test
    void testAgregarBloqueOnLogP() {
        int[] usuariosArray = {1000, 10000, 100000};
        int[] transaccionesArray = {1000, 10000, 100000};

        for (int pId = 0; pId < usuariosArray.length; pId++) {
            int P = usuariosArray[pId];
            Berretacoin sistema = new Berretacoin(P);
            for (int nId = 0; nId < transaccionesArray.length; nId++) {
                int N = transaccionesArray[nId];
                Transaccion[] transacciones = new Transaccion[N];
                for (int i = 0; i < N; i++) {
                    transacciones[i] = new Transaccion(i, (i % P) + 1, ((i + 1) % P) + 1, 1);
                }
                long start = System.currentTimeMillis();
    ...
    void testTxUltimoBloqueEsOn() {
        ...
        for (int id = 0; id < casos.length; id++) {
            ...
            Transaccion[] transacciones = new Transaccion[n];
            for (int i = 0; i < n; i++) {
                transacciones[i] = new Transaccion(i, (i % n) + 1, ((i + 1) % n) + 1, 1);
            }
            sistema.agregarBloque(transacciones);
            long start = System.nanoTime();
            Transaccion[] resultado = sistema.txUltimoBloque();
            long end = System.nanoTime();
            System.out.println("txUltimoBloque() con " + n + " transacciones -> " + (end - start) + "ns");
            assert resultado.length == n : "La cantidad de transacciones devueltas debe ser " + n;
        }
    }

    @Test
    void testMaximoTenedorEsO1() {
        int[] casos = {1000, 10000, 100000, 1000000};
        for (int id = 0; id < casos.length; id++) {
            int n = casos[id];
            Berretacoin sistema = new Berretacoin(n);
            ...
            for (int i = 0; i < 1000000; i++) {
                sistema.maximoTenedor();
            }
            long end = System.nanoTime();
            System.out.println("maximoTenedor() x1.000.000 con " + n + " usuarios -> " + (end - start) + "ns");
        }
    }
    
    @Test
    void testMontoMedioUltimoBloqueEsO1() {
        int[] casos = {1000, 10000, 100000, 1000000};
        for (int id = 0; id < casos.length; id++) {
            int n = casos[id];
            Berretacoin sistema = new Berretacoin(n);
            ...
            long start = System.nanoTime();
            for (int i = 0; i < 1000000; i++) {
                sistema.montoMedioUltimoBloque();
            }
            long end = System.nanoTime();
            System.out.println("montoMedioUltimoBloque() x1.000.000 con " + n + " transacciones -> " + (end - start) + "ns");
        }
    }

    @Test
    void testHackearTxEsOlogNxP() {
        ...
        for (int pId = 0; pId < usuariosArray.length; pId++) {
            ...
            for (int nId = 0; nId < transaccionesArray.length; nId++) {
                ...
                sistema.hackearTx();
                long end = System.nanoTime();
                System.out.println("hackearTx() con P=" + P + " usuarios y N=" + N + " transacciones -> " + (end - start) + "ns");
            }
        }
    }
}
